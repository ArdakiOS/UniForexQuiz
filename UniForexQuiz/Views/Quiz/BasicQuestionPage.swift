...
struct BasicQuestionPage: View {
    @ObservedObject var vm : BasicQuizVM
    ...
    var body: some View {
        ZStack{
            VStack{
                HStack{
                    Text("Basic Quiz Lvl \(vm.quizArray[vm.idx].lvl)")
                        .font(.system(size: 24))
                        .fontWeight(.bold)
                    Spacer()
                    Button {
                        ...
                        Button {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 1){
                                ...
                                if vm.idx + 1 == vm.quizArray.count{
                                    vm.saveCurrentQuestionIndex(vm.idx, lvl: vm.quizArray[vm.idx].lvl)
                                    showQuiz = false
                                    UserDefaults.standard.setValue(true, forKey: "completeBasic")
                                }
                                else{
                                    line += 1
                                    vm.idx += 1
